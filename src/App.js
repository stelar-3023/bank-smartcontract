...
function App() {
  ...
  const checkIfWalletIsConnected = async () => {
    try {
      if (window.ethereum) {
        ...
        setError('Please install a MetaMask wallet to user our bank.');
  ...
  const setBankNameHandler = async (event) => {
    ...
    try {
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum); // providers can only do read-only transactions
        const signer = provider.getSigner();
        const bankContract = new ethers.Contract(
          contractAddress,
          contractABI,
